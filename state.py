...
class File(object):
    """
    is_generated: is the file generated by redo ?
    is_override: has the file been overriden outside of redo ?
    checked_runid: <should be abstracted>
    changed_runid: <should be abstracted>
    failed_runid: <should be abstracted>
    stamp:
    csum:
    """
    ...
    def has_failed(self):
        "is the file failed (this run or a previous one)"
        return self.failed_runid

    def is_failed(self):
        "is the file failed from current build"
        return self.failed_runid and self.failed_runid >= vars.RUNID

    def is_out_of_date(self, runid):
        return self.changed_runid > runid
    ...
    def read_stamp(self):
        """
        Return the current timestamp of the file
        STAMP_MISSING: when the file does not exists
        STAMP_DIR:     when the file is a directory
        
        Note: two stamps can't be equal except if they are about the same file
        """
        try:
