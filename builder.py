    ...
    def _start_do(self):
        ...
        if (sf.is_generated and
            ...
            (sf.stamp != newstamp or sf.is_override)):
                if vars.OVERWRITE:
                    state.warn_override(sf.nicename(), overwrite=True)
                else:
                    state.warn_override(_nice(t), overwrite=False)
                    sf.set_override()
                    sf.set_checked()
                    sf.save()
                    return self._after2(0)
        if (os.path.exists(t) and not os.path.isdir(t + '/.')
            ...
            if vars.OVERWRITE:
                state.warn_existing(sf.nicename(), overwrite=True)
            else:
                state.warn_existing(sf.nicename(), overwrite=False)
                debug2("-- static (%r)\n" % t)
                sf.set_static()
                sf.save()
                return self._after2(0)
